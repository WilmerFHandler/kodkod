...
use crate::{
    AssistantMessage, Conversation, Message, Provider, ProviderError, ToolCall, ToolResult,
...
impl OpenAiCompatibleProvider {
    pub fn new(base_url: impl Into<String>, model: impl Into<String>) -> Self {
        Self::with_client(reqwest::Client::new(), base_url, None, model)
    }
...
impl Provider for OpenAiCompatibleProvider {
    async fn complete(
        ...
        let request = ChatCompletionRequest::from_agent_input(&self.model, conversation, tools)?;
...
impl ChatMessage {
    fn system(content: &str) -> Self {
        Self::new("system", Some(content.to_owned()))
    }

    fn new(role: &'static str, content: Option<String>) -> Self {
        Self {
            role,
            content,
            tool_calls: Vec::new(),
    ...
    fn from_message(message: &Message) -> Result<Self, ProviderError> {
        match message {
            Message::System(message) => Ok(Self::new("system", Some(message.content().to_owned()))),
            Message::User(message) => Ok(Self::new("user", Some(message.content().to_owned()))),
            Message::Assistant(message) => {
                let mut chat_message = Self::new("assistant", Some(message.content().to_owned()));
...
mod tests {
    ...
    use super::*;
    use crate::{ToolExecutorError, UserMessage};
